...
#[command]
async fn system_design_definition(ctx: &Context, msg: &Message) -> CommandResult {
let definition = "
시스템이 특정 요구사항을 충족하도록,
...
#[command]
async fn considerations(ctx: &Context, msg: &Message) -> CommandResult {
  let cons = "
...
- ETC...
";
  msg.channel_id.say(&ctx.http, cons).await?;
  Ok(())
}

#[group]
#[commands(system_design_definition, considerations)]
